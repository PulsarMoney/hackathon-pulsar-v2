...
import { TransferService } from './endpoints/transfer/transfer.service';
...
async function bootstrap() {
  ...
  const transferService = publicApp.get<TransferService>(TransferService);
  ...
  logger.log(`Owner address present: ${apiConfigService.getOwnerAddress()}`);
  logger.log(`Fee address present: ${await transferService.getFeeAddress()}`);
  logger.log(`Contract present: ${apiConfigService.getPulsarPageContract()}`);
}
