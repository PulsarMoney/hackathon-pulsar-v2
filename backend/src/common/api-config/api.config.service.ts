...
import { TransferService } from 'src/endpoints/transfer/transfer.service';
...
export class ApiConfigService {
  ...
  getOwnerAddress(): string {
    const ownerAddress = this.configService.get<string>('addresses.ownerAddress');
    if (!ownerAddress) {
      throw new Error('No owner address present');
    }
    return ownerAddress;
  }

  getPulsarPageContract(): string {
    const contract = this.configService.get<string>('contracts.pulsarPage');
    if (!contract) {
      throw new Error('No contract present');
    }
    return contract;
  }
