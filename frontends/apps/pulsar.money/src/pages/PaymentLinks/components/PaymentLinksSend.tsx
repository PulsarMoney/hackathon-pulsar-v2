...
export const PaymentLinksSend = () => {
  ...
  const avalabileTokens = useUserStore((s) => s.data.tokens);
  ...
  const getBalance = (selectedToken: string) => {
    const token = avalabileTokens.find((t) => t.identifier === selectedToken);
    return token.amount.toFixed(5) ?? 0;
  };
  ...
  const onSubmit = async (data: CreatePaymentLinkForm) => {
    if (Number(data.amount) > Number(getBalance(selectedToken))) {
        ...
        <form className="max-w-xs mt-4 flex flex-col gap-4 " onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <div>
            ...
            <span className="text-gray-500 text-xs">
              Avalabile: {getBalance(selectedToken)} {selectedToken}
            </span>
          </div>
          ...
          <Button variant="default" type="submit" disabled={!form.getValues("terms") === true || createPaymentLink.isLoading}>
